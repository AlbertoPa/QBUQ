...
def ask_post_job():
    ...
    if job == '1':
        ...
        if len(weights) != Nsample:
            print("The number of samples does not match number of weights. " \
            "Program exits.")
            exit()
        ...
        print("The PDF will be reconstructed.")
...
def ask_maxMomentOrder():
    ...
    if str.lower(n) == 'q':
        ...
        print("Please enter a positive integer. Press q to quit.")
...
def ask_sample_number():
    ...
    if str.lower(n) == 'q':
        ...
        print("Number of samples cannot be 0. Please enter a positive "\
...
def ask_node_number():
    ...
    if str.lower(n) == 'q':
        ...
        print("Number of nodes cannot be 0. Please enter a positive "\
              "integer. Press q to quit.")
        n = ask_node_number()
    num = int(n)
    if num > 5:
...
def ask_para_Wheeler(N):
    ...
    print("Please enter "+str(N)+" ratios of minimum weights to maximum "\
      "weights (rmin). See the documentation for details about these ratios.")
    rmin = zeros(N)
    for i in range(N):
        print("Please enter rmin["+str(i)+"].")
        rmin[i] = ask_num()
    print("Please enter the minimum distance between distinct nodes (eabs). " \
          "See the documentation for details about this parameter.")
    eabs = ask_num()
    return rmin, eabs
