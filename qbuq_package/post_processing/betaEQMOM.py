...
def beta_EQMOM_general(weights, v, a, b, nnodes, rmin, eabs):
    ...
    vn = (v-a)/(b-a)
    # Estimate moments using normalized data
    m = zeros(2*nnodes+1)
    for i in range(len(m)):
        for j in range(len(vn)):
            m[i] += weights[j]*vn[j]**i
    m = m/m[0]
    # Use beta EQMOM on [0, 1] to determine nodes, weights, and sigma. 
    w, x, sig, nout, flag = beta_EQMOM(m, nnodes, rmin, eabs)
    # Transform nodes back to [a, b]
    xout = (b-a)*x+a
    return w, x, xout, sig, nout, flag

def beta_EQMOM(mom, nnodes, rmin, eabs):
    """ Return weights, nodes, sigma, number of nodes, and iteration number 
    using beta EQMOM on bounded interval [0,1].
    """
    ...
    nmom = 2*n+1
    if len(mom) < nmom:
        print("Not enough moments to generate "+str(n)+" nodes. Program exits.")
        exit()
    else:
        mom1 = mom[:nmom]
    m = mom1.copy()
    ...
    ms = beta_m2ms(m, sig, nmom-1)
    fold = beta_targetfun(m, sig, n, nnodes, rmin, eabs)
    ...
    if abs(fold) < ftol:
        ...
        Hdet = beta_targetHdet(m, sig, n)
        if  Hdet < -macheps:
            sig = beta_zbrent(m, n, sigposi[0], sig, ztol)
        elif Hdet >= -macheps and detflag == 0:
            sigposi.append(sig)
            posinum += 1
        snew = sig;
        fnew = beta_targetfun(m, snew, n, nnodes, rmin, eabs)
    ...
    if abs(fnew) < ftol:
        ...
        msnew = beta_calc_ms(w, x, 2*nnodes+1)
        mnew = beta_ms2m(msnew, sig, 2*nnodes+1)
    ...
    for j in range(1, itmax+1):
        ...
        Hdet = beta_targetHdet(m, sig, noutT)
        if Hdet < -macheps and detflag == 0:
            # Use the largest sigposi as lower bound of Brent's method.
            sig_low = max(sigposi)
            sig_low = min(sig_low, snew)
            sig_det = beta_zbrent(m, n, sig_low, sig, ztol)
            ms = beta_m2ms(m, sig_det, nmom-1)
            w, x, _, _ = Wheeler_moments_adaptive(ms[:2*n], n, rmin, eabs)
            msnew = beta_calc_ms(w, x, 2*nnodes+1)
            mnew = beta_ms2m(msnew, sig_det, 2*nnodes+1)
            normmdiff_det = norm((mnew-m)/m)
            detflag = 1
        #Compute new value of f
        f = beta_targetfun(m, sig, noutT, nnodes, rmin, eabs)
        df = f-fold
        ...
        if abs(f) < ftol or abs(ds) < stol:
            if detflag == 1:
                ...
                w, x, _, _ = Wheeler_moments_adaptive(ms[:2*n], n, rmin, eabs)
                msnew = beta_calc_ms(w, x, 2*nnodes+1)
                mnew = beta_ms2m(msnew, sigtemp, 2*nnodes+1)
                mdiff = (mnew-m)/m
                normmdiff = norm(mdiff)
                Hdet = beta_targetHdet(m, sigtemp, n)
            ...
            if normmdiff > mtol:
                ...
                print("mdiff = (mnew - m)/m = "+str(mdiff))
                print("norm(mdiff) = "+str(normmdiff))
                print("Moments are not conserve.")
            return w, x, sig, nout, flag
        if dsmax == 0 and f > 0:
...
def beta_targetfun(m, sig, n, nnodes, rmin, eabs):
    ...
    ms_temp = beta_calc_ms(w, x, nmom)
    m_temp = beta_ms2m(ms_temp, sig, nmom)
    f = (m[nmom-1]-m_temp[nmom-1])/m[nmom-1] #Normalized to avoid small m[nom]
    if n < nnodes:
        nn = 2*nnodes+1
        ms = beta_m2ms(m, sig, 2*nnodes)
        w, x, _, _ = Wheeler_moments_adaptive(ms, nnodes, rmin, eabs)
        ms_temp = beta_calc_ms(w, x, nn)
        m_temp = beta_ms2m(ms_temp, sig, nn)
        fold = 0
        for i in range(nn):
...
def beta_zbrent(m, n, x1, x2, tol):
    ...
    b = x2
    c = x2
    fa = beta_targetHdet(m, a, n)
    fb = beta_targetHdet(m, b, n)
    if fa*fb > 0:
